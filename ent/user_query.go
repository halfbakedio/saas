...
func (uq *UserQuery) QueryTenant() *OrganizationQuery {
	...
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		...
		step := sqlgraph.NewStep(
			...
			sqlgraph.Edge(sqlgraph.O2M, false, user.TenantTable, user.TenantColumn),
		)
...
func (uq *UserQuery) WithTenant(opts ...func(*OrganizationQuery)) *UserQuery {
	query := (&OrganizationClient{config: uq.config}).Query()
...
func (uq *UserQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*User, error) {
	...
	if query := uq.withTenant; query != nil {
		if err := uq.loadTenant(ctx, query, nodes,
			func(n *User) { n.Edges.Tenant = []*Organization{} },
			func(n *User, e *Organization) { n.Edges.Tenant = append(n.Edges.Tenant, e) }); err != nil {
			return nil, err
		}
	}
...
func (uq *UserQuery) loadTenant(ctx context.Context, query *OrganizationQuery, nodes []*User, init func(*User), assign func(*User, *Organization)) error {
	...
	query.Where(predicate.Organization(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(user.TenantColumn), fks...))
	}))
	...
	for _, n := range neighbors {
		fk := n.user_tenant
		if fk == nil {
			return fmt.Errorf(`foreign-key "user_tenant" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "user_tenant" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
